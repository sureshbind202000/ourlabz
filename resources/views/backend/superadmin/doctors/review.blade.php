...
<div class="card mb-3">
    <div class="card-header">
        <div class="row flex-between-end">
            <div class="col-auto align-self-center">
                <h5 class="mb-0" data-anchor="data-anchor">All Reviews</h5>
            </div>
        </div>
    </div>
    <div class="card-body pt-0">
        <div class="tab-content">
            ...
            <div id="tableExample3" data-list='{"valueNames":["doctor","name","email","rating"],"page":10,"pagination":true}'>
                    ...
                    <table class="table table-bordered table-striped fs-10 mb-0">
                        <thead class="bg-200">
                            <tr>
                                ...
                                <th class="text-900" data-sort="doctor">Doctor</th>
    ...
    $(document).ready(function() {
        ...
        function fetchReview() {
            Swal.fire({
             ...
             $.ajax({
        url: "{{ route('doctor.review.list') }}",
        type: "GET",
        success: function(data) {
            Swal.close();
            let rows = "";
            let filteredData = data.filter(review => review.user && review.user.id === authUserId);

            $.each(filteredData, function(index, review) {
                rows += `<tr>
                    <td>${index + 1}</td>
                    <td class="blog">
                        <a href="/doctor/${review.encrypted_id}/details" target="_blank">${review.user.name}</a>
                    </td>
            ...
            new List('tableExample3', {
                valueNames: ['doctor', 'name', 'email', 'date', 'rating'],
                page: 10,
                pagination: true
            });
        }
        ...
        $(document).on('change', '.status-toggle', function() {
            ...
            $.ajax({
                url: "{{ route('doctor.review.status') }}",
                type: "POST",
                data: {
                    id: reviewId,
                    status: status
