...
window.Dropzone ? Dropzone.autoDiscover = false : '';
...
var dropzoneInit = function () {
  ...
  const dropzoneElement = document.querySelector(Selector.DROPZONE);
  if (!dropzoneElement) return;

  const previewTemplateNode = dropzoneElement.querySelector(Selector.DZ_PREVIEW);
  const previewTemplate = previewTemplateNode.innerHTML;
  previewTemplateNode.innerHTML = '';

  dropzoneInstance = new Dropzone(dropzoneElement, {
    url: "#", // not uploading yet
    autoProcessQueue: false,
    uploadMultiple: true,
    parallelUploads: 5,
    maxFilesize: 5, // MB
    acceptedFiles: "image/*",
    previewsContainer: previewTemplateNode,
    previewTemplate: previewTemplate,
    addRemoveLinks: true,
    init: function () {
      // No auto uploads, just prepare queue
    }
  });
};
...
var wizardInit = function wizardInit() {
  ...
  wizards.forEach(function (wizard) {
    ...
    var submitEvent = new Event(events.SUBMIT, { bubbles: true, cancelable: true });
    ...
    if (tabToggleButtonEl.length) {
      tabToggleButtonEl.forEach(function (item, index) {
        ...
        item.addEventListener(events.SHOWN, function () {
          ...
          // Always enable Previous and Next buttons
          prevButton.classList.remove('d-none');
          nextButton.classList.remove('d-none');

          // Add "done" and "complete" classes dynamically
