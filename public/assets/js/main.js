...
(function ($) {
    ...
    $(window).scroll(function () {
        if ($(this).scrollTop() > 100) {
            ...
            // $('.list-item-info').addClass("d-none");
            // $('.navbar-nav ').removeClass("p-16");

        } else {
            $('.navbar').removeClass("fixed-top");
            // $('.list-item-info').removeClass("d-none");
            // $('.navbar-nav ').addClass("p-16");
        }
    });
  ...
  function highlightActivePage() {
    const path = location.pathname.replace(/\/$/, ''); // Current path without trailing slash
    const links = document.querySelectorAll('#sidebar-list li a');

    links.forEach(link => {
      link.classList.remove('active'); // Remove active class

      const href = new URL(link.href, window.location.origin).pathname.replace(/\/$/, ''); // Normalize href path

    //   console.log(`Checking href: ${href} === path: ${path}`); // Debugging line

      if (href === path) {
        link.classList.add('active'); // Add active class if matches
      }
    });
  }
